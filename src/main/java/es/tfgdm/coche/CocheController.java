...
public class CocheController {
	...
	public ModelAndView coches(@RequestParam(defaultValue = "0") int page) {
		...
		model.addObject("totalPages", listaCoches.getTotalPages());
		return model;
	}
	...
	public ModelAndView formCoche(@ModelAttribute @Valid Coche coche, BindingResult bindingResult,
		...
		if (!updating && cocheDAO.existsById(coche.getId())) {
			bindingResult.rejectValue("id", "error.coche", "El ID ya existe");
			model.setViewName("coches/cocheForm");
			model.addObject("coche", coche);
			model.addObject("updating", updating);
			return model;
